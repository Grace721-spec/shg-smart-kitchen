...
# Run initial target calculations
...
if menu_choice == "Githeri":
    st.error("⚠️ **AI Alert:** Portions slashed by 40%. High number of students expected to skip lunch or eat buns!")
elif menu_choice in ["Ugali & Bitterherbs", "Ugali & Cabbage"]:
    st.warning("⚠️ **AI Alert:** Portions reduced by 35%. Students heavily prefer the canteen today!")
...
st.write("Enter how much food was thrown away yesterday. The AI will automatically subtract this from today's store harvest to prevent over-cooking.")

col_waste1, col_waste2 = st.columns(2)
with col_waste1:
    yesterday_waste_kgs = st.number_input("Yesterday's Thrown Away Food (in KGs):", min_value=0.0, max_value=500.0, value=0.0, step=1.0)

# --- INGREDIENT WEIGHT PLANNER ---
st.write("### ⚖️ Raw Ingredient Weight Planner")
st.write("Adjust how many grams of each raw ingredient go onto a single plate. The AI calculates the total KGs, minus the waste.")
...
def calculate_net_kgs(total_portions, grams_per_plate, waste_reduction_kgs):
    ...
    # Subtract waste evenly or cap it at 0 so it never goes negative
...
with col1:
    ...
    # If it's the main carb, we subtract yesterday's waste directly from it
    ing1_kgs, ing1_base = calculate_net_kgs(portions, ing1_grams, yesterday_waste_kgs)
    st.metric(label=f"Total {ing1_name} Needed", value=f"{ing1_kgs} KGs", delta=f"-{yesterday_waste_kgs} KG Waste Trim" if yesterday_waste_kgs > 0 else None)

with col2:
    ing2_name = st.text_input("Ingredient 2 Name:", value="Protein/Legume (e.g., Beans)")
    default_prot = 100 if menu_choice not in ["Ugali & Bitterherbs", "Ugali & Cabbage"] else 0
    ing2_grams = st.number_input("Grams per plate (Ing 2):", min_value=0, max_value=1000, value=default_prot, step=10)
    # Scale down protein waste proportionally (e.g., half the weight of carb waste)
    ing2_kgs, ing2_base = calculate_net_kgs(portions, ing2_grams, yesterday_waste_kgs * 0.5)
